...
class UserByToken(Resource):
    ...
    def put(self):
        ...
        result = execsql("select id,username,salt,hash,email,privileges+0 from users where id=%d", (user_id,))
        if result[0] == []:
            abort(400, message="The user of this token does not exist")
        ...
        if args["username"] is not None and len(args["username"]) > 0:
            ...
            sql_result = execsql("select username from users where username like %s and id != %d", (args["username"],user_id))
            if sql_result != []:
                abort(400, message="Username already exists: {}".format(args["username"]))
        ...
        execsql("update users set username=%s,salt=%s,hash=%s,email=%s where id=%d", (username, salt, hash, email, user_id))
        # else:
            # execsql("update users set privileges=%d where id=%d", (privileges, user_id))
